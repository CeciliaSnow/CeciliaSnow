<template>
  <div class="index">
    ...
    <div style="float:right;padding-right:50px">
          <el-button  @click=" this.$router.push('/accepted')" type="primary">返回</el-button>
          </div>
        ...
        <el-button  type="info" disabled>等待雇主审核</el-button><br>
         <el-button style="margin-top:30px" type="primary" @click="qq()">取消接取</el-button>
         </div>
         <h4>{{user.name}}</h4>
      </div>
...
export default {
  ...
  methods: {
      load(){
          ...
          this.user2 = JSON.parse(userStr)
          axioss.post("/position/accbyid/"+this.key).then(res=>{
              if(res.data.code ==='0'){
              this.position = res.data.data
              document.getElementById("zhi").innerHTML=this.position.content
                axioss.post("/user/querybyid/"+this.position.userid).then(res=>{
                   if(res.data.code ==='0'){
      ...
      qq(){
        axioss.post("/work/qq/"+this.position.id+"/"+this.user2.id).then(res=>{
          if(res.data.code==="0"){
             this.$message({
                  type:'success',
                  message:"成功"
