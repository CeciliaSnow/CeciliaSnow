...
<el-dialog v-model="dialogFormVisible" title="">
  ...
  <el-dialog v-model="dialogFormVisible1" title="">
    <el-form :model="positioninfo">
      ...
      <el-form-item label="负责人" :label-width="formLabelWidth">
        <el-input v-model="positioninfo.people" autocomplete="off" style="width:20%"></el-input>
      </el-form-item>
      <el-form-item label="联系方式" :label-width="formLabelWidth">
        <el-input v-model="positioninfo.phone" autocomplete="off" style="width:30%"></el-input>
      </el-form-item>
      <el-form-item label="联系邮箱" :label-width="formLabelWidth">
        <el-input v-model="positioninfo.email" autocomplete="off" style="width:30%"></el-input>
      </el-form-item>
      <div id="div2"></div>
    </el-form>
    <template #footer>
      ...
      <el-button style="float:left" type="primary" @click="dell(positioninfo.id)">取消发布</el-button>
        ...
        <el-button type="primary" @click="infod()">更改</el-button>
      </span>
    </template>
  </el-dialog>
</template>
  ...
  data() {
    ...
    methods: {
        load(){
          ...
          if(this.user){
                axioss.post("/temp/querytemp/"+this.user.id).then(res=>{
                  if(res.data.code === "0"){
        ...
        infod(){
          if(this.user.id){
            ...
            axioss.put("/temp/update",this.positioninfo).then(res=>{
              if(res.data.code=="0"){
                this.$message({
                      type:"success",
                      message: "成功"
        ...
        dell(id){
           this.dialogFormVisible1 = false
           axioss.get('/temp/no/'+id).then(res=>{
            if(res.data.code === "0"){
                this.load()
                this.$message({
                type: "success",
