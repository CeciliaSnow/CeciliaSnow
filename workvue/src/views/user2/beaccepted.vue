<template>
  ...
  <ul v-if="position!=''" v-infinite-scroll="load" class="infinite-list" style="">
...
export default {
  ...
  methods: {
    load() {
      ...
      axioss.post("/position/beaccepted/" + this.user.id).then((res) => {
        if (res.data.code === "0") {
    ...
    info(key){
      this.$router.push('/beaccepted2/'+key)
    }
  },
};
</script>